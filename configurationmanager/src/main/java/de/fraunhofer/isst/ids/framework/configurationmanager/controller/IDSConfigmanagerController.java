...
public class IDSConfigmanagerController implements IDSConfigmanagerAPI {
    ...
    public ResponseEntity<String> updateConfig(ConnectorUpdateMessage connectorUpdateMessage) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Could not deserialize the received configuration to ConfigurationModel class!");
        }catch (ConfigurationUpdateException e){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Could not load Key- and Truststore with new configuration, rejected Config!");
        }
